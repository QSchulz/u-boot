...
def test_env_import_whitelist(state_test_env):
    """Test exporting and importing environments."""
    c = state_test_env.u_boot_console
    ...
    c.run_command('printenv')
    #those makes the env import function to fail
    c.run_command('setenv -f ethaddr')
    c.run_command('setenv -f eth1addr')
    c.run_command('setenv -f eth2addr')
    c.run_command('setenv -f eth3addr')
    c.run_command('setenv -f eth4addr')
    c.run_command('setenv -f eth5addr')
    validate_set(state_test_env, 'foo1', 'bar1')
    validate_set(state_test_env, 'foo2', 'bar2')
    validate_set(state_test_env, 'foo3', 'bar3')

    c.run_command('md %s' % addr)
    ...
    c.run_command('env import %s' % addr)
